...
const PrintableInvoiceContent: React.FC<{ invoice: SalesInvoice | null; state: AppState }> = ({ invoice, state }) => {
    ...
    const calculateItemValue = (item: InvoiceItem) => {
        // Logic: Rate is treated as Per Unit/Package
...
const SalesInvoiceModule: React.FC<SalesInvoiceProps> = ({ setModule, userProfile }) => {
    ...
    const handleAddItem = (e: React.FormEvent) => {
        ...
        // Direct Package Rate save
        let rateToSave = Number(currentPackageRate);
        if (rateToSave <= 0) {
            rateToSave = itemDetails.avgSalesPrice;
        }
                ...
                <form onSubmit={handleAddItem} className="space-y-4">
                    <div>
                        ...
                        <ItemSelector
                            ...
                            onSelect={(id) => {
                                ...
                                setCurrentPackageRate(selectedItem?.avgSalesPrice || '');
                            }}
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full p-2 rounded-md"
                            placeholder={itemDetails ? `e.g., Rate for ${itemDetails.name}` : "Rate"}
                        />
                    </div>
                                ...
                                <table className="w-full text-left table-auto">
                                    ...
                                    <tbody>
                                        {currentInvoiceItems.map((item, index) => {
                                            ...
                                            // Logic Update: Total Worth = Quantity * Rate.
                                            const totalWorth = quantity * rate * conversionRate;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <CollapsibleSection title="Additional Costs">
                             <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <input type="number" value={customCharges} placeholder="Custom Charges Amount" onChange={e => setCustomCharges(e.target.value === '' ? '' : Number(e.target.value))} disabled={!clearingAgentId} className="w-full p-2 rounded-md" />
